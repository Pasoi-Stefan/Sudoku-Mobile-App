...
public class TextViewWithImages extends androidx.appcompat.widget.AppCompatTextView {
    ...
    private static Spannable getTextWithImages(Context context, CharSequence text, int lineHeight) {
        final Spannable spannable = Spannable.Factory.getInstance().newSpannable(text);
        addImages(context, spannable, lineHeight);
        return spannable;
    }

    private static boolean addImages(Context context, Spannable spannable, int lineHeight) {
        ...
        while (matcher.find()) {
            ...
            if (set) {
                hasChanges = true;
                spannable.setSpan(makeImageSpan(context, id, lineHeight),
                        matcher.start(),
                        matcher.end(),
     ...
     */
    private static ImageSpan makeImageSpan(Context context, int drawableResId, int size) {
